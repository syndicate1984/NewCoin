  ...
  for k,v in pairs(tr.players) do
    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
		...
		local diamante_ok = (vRP.getDiamanteMoney(user_id) >= recipe.in_diamante)
        ...
        if money_ok and diamante_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          for l,w in pairs(recipe.reagents) do
            vRP.tryGetInventoryItem(user_id,l,w,true)
		          ...
		          -- Diamante
          if recipe.in_diamante > 0 then vRP.tryDiamantePayment(user_id,recipe.in_diamante) end
          for l,w in pairs(recipe.reagents) do
            vRP.tryGetInventoryItem(user_id,l,w,true)
          end
          -- produce products
          ...
          --Diamante
          if recipe.out_diamante > 0 then vRP.giveDiamanteMoney(user_id,recipe.out_diamante) end
          for l,w in pairs(recipe.products) do
            vRP.giveInventoryItem(user_id,l,w,true)
          end
          -- give exp
