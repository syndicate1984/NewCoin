...
local function ch_givediamantemoney(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
    vRP.prompt(player,"Amount:","",function(player,amount) 
      amount = parseInt(amount)
      vRP.giveDiamanteMoney(user_id, amount)
    end)
  end
end
      ...
      vRP.buildMenu("admin", {player = player}, function(menu)
        ...
        if vRP.hasPermission(user_id,"player.givemoney") then
          menu["Give money"] = {ch_givemoney}
		        ...
		        if vRP.hasPermission(user_id,"player.givediamante") then
          menu["Give Diamante"] = {ch_givediamantemoney}
        end
