...
MySQL.createCommand("vRP/money_init_user","INSERT IGNORE INTO vrp_user_moneys(user_id,wallet,diamante,bank) VALUES(@user_id,@wallet,@diamante,@bank)")
MySQL.createCommand("vRP/get_money","SELECT wallet,diamante,bank FROM vrp_user_moneys WHERE user_id = @user_id")
MySQL.createCommand("vRP/set_money","UPDATE vrp_user_moneys SET wallet = @wallet, diamante = @diamante, bank = @bank WHERE user_id = @user_id")
...
-- get diamante
-- cbreturn nil if error
function vRP.getDiamanteMoney(user_id)
  local tmp = vRP.getUserTmpTable(user_id)
  if tmp then
    return tmp.diamante or 0
  else
    return 0
  end
end
...
-- set diamante
function vRP.setDiamanteMoney(user_id,value)
  local tmp = vRP.getUserTmpTable(user_id)
  if tmp then
    tmp.diamante = value
  end
  ...
  local source = vRP.getUserSource(user_id)
  if source ~= nil then
    vRPclient.setDivContent(source,{"diamante",lang.money.ddisplay({value})})   -----
  end
end

 -- try a payment
-- return true or false (debited if true)
function vRP.tryDiamantePayment(user_id,amount)
  local money = vRP.getDiamanteMoney(user_id)
  if money >= amount then
    vRP.setDiamanteMoney(user_id,money-amount)
    return true
  else
    return false
  end
 ...
 -- give money
function vRP.giveDiamanteMoney(user_id,amount)
  local money = vRP.getDiamanteMoney(user_id)
  vRP.setDiamanteMoney(user_id,money+amount)
end
...
AddEventHandler("vRP:playerJoin",function(user_id,source,name,last_login)
  MySQL.execute("vRP/money_init_user", {user_id = user_id, wallet = cfg.open_wallet, diamante = cfg.open_diamante, bank = cfg.open_bank}, function(affected)
...
AddEventHandler("vRP:playerLeave",function(user_id,source)
  -- (wallet,bank)
  local tmp = vRP.getUserTmpTable(user_id)
  if tmp and tmp.wallet ~= nil and tmp.bank ~= nil and tmp.diamante ~= nil then
    MySQL.execute("vRP/set_money", {user_id = user_id, wallet = tmp.wallet, diamante = tmp.diamante, bank = tmp.bank})
  end
end)
...
AddEventHandler("vRP:save", function()
  for k,v in pairs(vRP.user_tmp_tables) do
    if v.wallet ~= nil and v.bank ~= nil and v.diamante ~= nil then
      MySQL.execute("vRP/set_money", {user_id = k, wallet = v.wallet, diamante = v.diamante, bank = v.bank})
    end
  end
end)
...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    vRPclient.setDiv(source,{"money",cfg.display_css,lang.money.display({vRP.getMoney(user_id)})})
	    vRPclient.setDiv(source,{"diamante",cfg.display_css,lang.money.ddisplay({vRP.getDiamanteMoney(user_id)})})
